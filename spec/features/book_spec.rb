...
RSpec.describe "Nulibrary", type: :feature, driver: :selenium do
  ...
  describe "Book details" do
    ...
    it "has borrow button when user is logged-in and inventory is greater than zero" do
      log_in_user(user_1)
    ...
    it "doesn't have borrow button when user is not logged-in" do
      visit book_path(book_1.id)

      expect(page).not_to have_selector('.borrow-button')
    end

    it "has total quantity and current quantity information" do
      visit book_path(book_1.id)
